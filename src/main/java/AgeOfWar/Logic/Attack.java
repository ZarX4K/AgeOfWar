...
public class Attack {
    ...
    private void executeDamageExchange(BaseCharacterStats attacker, BaseCharacterStats target) {
        ...
        if (target instanceof Tank) {
            ...
            if (reductionMode == DamageReductionMode.ARMOR_ACTIVATED) {
                ...
                target.takeDamage(targetDamage); // Critical hit, double damage
            } else {
    ...
    private int getDamageWithCritical(BaseCharacterStats character) {
        int damage = character.getDamage();  // Get the base damage
        int critDamage = 0;  // Default value for critical damage

        // If the character is a Knight, use Knight's critical damage
        if (character instanceof Knight) {
            Knight knight = (Knight) character;  // Cast to Knight
            if (knight.isCriticalHit()) {
                critDamage = knight.getCritical();  // Set critical damage to the Knight's critical value
                System.out.println(knight.getClass().getSimpleName() + " landed a critical hit! Critical damage: " + critDamage);
                return critDamage;  // Return the critical damage directly
            }
        }
        // If the character is a Tank, use Tank's critical damage
        else if (character instanceof Tank) {
            Tank tank = (Tank) character;  // Cast to Tank
            if (tank.isCriticalHit()) {
                critDamage = tank.getCritical();  // Set critical damage to the Tank's critical value
                System.out.println(tank.getClass().getSimpleName() + " landed a critical hit! Critical damage: " + critDamage);
                return critDamage;  // Return the critical damage directly
            }
        }

        // Default return value is the base damage if no critical hit
        return damage;
    }
